...
const SITE_ORIGIN = 'https://qctcommerce.com';
...
if (!urls.length) throw new Error('No QCT Commerce URLs found in the sitemap.');
const batches = [];
for (let index = 0; index < urls.length; index += BATCH_SIZE) batches.push(urls.slice(index, index + BATCH_SIZE));
console.log(`Preparing ${urls.length} unique QCT Commerce URLs in ${batches.length} batch(es)${LOCAL_SITEMAP ? ' from local sitemap files' : ''}.`);
