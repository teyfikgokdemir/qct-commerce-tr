...
for (const file of htmlFiles) {
  ...
  const expectedLang = pathname.startsWith('/en/') ? 'en' : 'tr';
  ...
  if (titleTags.length !== 1 || !titleTags[0].replace(/<\/?title>/gi, '').trim()) {
    errors.push(`${file}: expected one non-empty title.`);
  }
  ...
  if (canonicalTags.length !== 1 || attribute(canonicalTags[0], 'href') !== canonical) {
    errors.push(`${file}: canonical does not match ${canonical}.`);
  }
  ...
  if (!nodes.some((node) => ['WebPage', 'AboutPage', 'ContactPage', 'CollectionPage'].includes(node['@type']))) {
