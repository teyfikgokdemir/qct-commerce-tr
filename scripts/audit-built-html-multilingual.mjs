...
function tags(html, name) { return html.match(new RegExp(`<${name}\\s+[^>]*>`, 'gi')) ?? []; }
...
function localPageExists(pathname) {
  ...
  const candidates = pathname.endsWith('/') ? [path.join(DIST, clean, 'index.html')] : [path.join(DIST, clean), path.join(DIST, `${clean}.html`), path.join(DIST, clean, 'index.html')];
...
for (const file of htmlFiles) {
  ...
  const nodes = [];
  for (const match of jsonLdMatches) { try { nodes.push(...schemaNodes(JSON.parse(match[1].trim()))); } catch (error) { errors.push(`${file}: invalid JSON-LD (${error.message}).`); } }
  ...
  for (const tag of tags(html, 'a')) {
    ...
    let url;
    try { url = new URL(href, SITE); } catch { errors.push(`${file}: invalid link ${href}.`); continue; }
...
else console.log('Multilingual canonical, metadata, schema and internal-link audit passed.');